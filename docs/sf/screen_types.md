...
2.  Select the relevant tabel(s) and variant(s) in the *Assign to subjects* tab to which this screen type must be assigned.
...
To design a custom screen type a new screen type must first be created in the *Screen type* overview. The id and possibly a definition of this new screen type is specified here.
...
| ![1538569788467](../assets/sf/1538569788467.png) |      Grid      | The data is displayed under each other in the grid and it can be selected. The user interface will remove the grid if there can only be at most one line. |
...
| ![1538569955237](../assets/sf/1538569955237.png) | Tree structure | Displays a tree structure. The user interface will remove the structure if there can only be at most 1 row. |
