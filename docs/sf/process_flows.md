...
| Command delimiter (regex) | Optional. A C# regular expression used to instruct the connector to execute multiple sequential commands on the same connection.<br>For example:<code>`;[^;]*(?:\z|--go\r\n|--go\n|--GO\r\n|--GO\n)</code><br> The _SQL_ value will be split into multiple commands using this regular expression.
