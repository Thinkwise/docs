...
The organizing of roles is now moved out of IAM, to the Software Factory. This way, … For this purpose, a new list bar group is created, named ‘Access Control’.

One of the screens present in this list bar group is the role organizing screen ‘Roles’. In here, it is possible to execute a task to import the roles out of IAM, to the software factory. It is also possible to create or copy new roles, or to delete roles. Assigning role rights to objects can be done in the same way as you were used to in IAM. 

This also goes for the ‘Model rights’ screen. This screen is optimized by using another screen type, which makes better use of the available space. In this screen you can also find the tabpage ‘New objects’, which shows all new objects created in the project version. There is a ‘Go to object’ task available, which leads the developer to the corresponding record in the ‘Model rights’ screen.

### Synchronization and deployment

The action of synchronizing between the Software Factory and IAM has been moved to the Software Factory. This way, … For this purpose, two list bar item are added to this list bar group.

One of the screens present in this list bar group is ‘Synchronize to IAM’. Not only is it possible to start a synchronization to IAM, but you can also write a file to disk. Before starting the synchronization there will be a couple of checks. In some cases this will lead to a warning (not all validations are solved), and in some cases this will lead to not being able to execute the synchronization (no roles are present in the project version).

The second new screen in this list bar group is ‘Deployment package’. In here you can create a deployment package. This will lead to the project version being generated, to be synchronized to IAM, and the creation of the deployment package. 
...
It is now possible to set if a validation may be approved or not. By default it is not possible to approve errors, bus this can be changed. This also goes the other way around. By default is is possible to approve information messages or warnings, bus this can also be changed.
...
* **Copy translations when renaming an object** - Translations will be copied automatically when renaming an object.
