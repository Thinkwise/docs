...
This screen also contains the tabpage *New objects*, which shows all new objects created in the project version. There is a *Go to object* task available, which leads to the corresponding record in the *Model rights* screen.
...
The second new screen in this menu group is *Deployment package*. This screen looks a lot like the *Synchronize to IAM* screen, only in here a deployment package can be created. After execution, the following actions will be executed:
...
It is now possible to determine the starting order of applications (runtime configurations) when starting Universal againts the Software Factory, without setting specific applications (runtime confirations).
...
* **Grid link task id extender** - Allows a task to be executed by clicking on a field in the grid.
...
Made the following screens more user friendly:
* Adding conditional layout colours: variant columns, form columns and grid columns.
* Prefilters, tasks and reports are now available in a grouped grid. This also applies to the corresponding variants. The grid is default editable and provides the option to use drag-drop.
* Added a *Process actions* tabpage in the tasks and reports screens.
*  Added a prefilter that only shows your own projects in the project screen and project version selection ribbon tab. By default this prefilter is activated, but it can be deactivated to show all projects.
* Added prefilter *Originated in current version* to code review screen.
* Added prefilter *Not used* to domain screen.
* All process actions now show the id it belongs to, so when multiple process actions of the same type are used they can be easily distinguished in the designer.
* The icons screen has been moved to the User Interface menu group. In here it has been made possible to alter multiple icons at once with a task.
* Templates in the *Available Templates* tabpage under *Functionality* are no longer alphabetically sorted, but sorted by the order number of the templates.
* Changed the translation for the column type domain element *Regular* to *Editable* to more easily distinguish them for autocomplete purposes.
...
* **Domain col_access_type** - The order of the elements has been changed for the col_access_type domain in IAM. From now on, the order is the same as for the same domain in the Software Factory.
