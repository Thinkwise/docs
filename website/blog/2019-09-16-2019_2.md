...
- [Projects](#projects)
  - [Requirements & Design 3.0](#requirements-&-design-3.0)
...
- [Software Factory](#software-factory)
  - [New](#new-sf)
    ...
    - [Show badge for parameters for assigned templates](#show-badge-for-parameters-for-assigned-templates) 
...
![Business processes](/docs/blog/assets/2019_2_requirements_design_business_processes.png)

### Unit testing & Mock data

Unit test now support processes and tasks. Data sets can be selected as mock data for the unit test.
...
![Unit test mock data](/docs/blog/assets/2019_2_unit_test_mock_data.png)

It is now also possible to use an assertion query to check the unit test results for expected outcome. 

![Unit test assertions](/docs/blog/assets/2019_2_unit_test_assertions.png)
...
The explanation of conditional layout can now be shown in the help. Translation objects are now generated for conditional layout. 
When the help text is filled with an explanation of the conditional layout, it will be shown in the help.

![Conditional layout help text](/docs/blog/assets/2019_2_conditional_layout_help_text.png)

### Module authorization
A feature that allows the creation of modules comprised of one or more roles that can be deployed individually. Modules can be defined in the role moduler under the "Module" menu item under "Access control". Roles can be added to modules by dragging and dropping them onto the modules. Once created, a module can be selected on the "Synchronize to IAM" or "Deployment package" screens. This will synchronize or deploy only the functionality contained within the roles in the selected modules.

![Module authorization](/docs/blog/assets/2019_2_module_authorization.png)

### Show badge for parameters for assigned templates
...
![Mandatory exclusive prefilter groups](/docs/blog/assets/2019_2_mandatory_exclusive_prefilter_groups.png)
...
### Cubes in menu
Based on feedback from our users, we dediced to reintroduce the cube-screen in the menu. The screen is placed on its old place in User Interface.
 
### Concurrent access limitations
A new feature is added to restrict the number of concurrent sessions per account. This can be set per application. 

![Concurrent access limitations](/docs/blog/assets/2019_2_concurrent_access_limitations.png)
...
In here the statistics are shown of how many validations or code reviews are assigned to a developer.

![Validations assignments tab page](/docs/blog/assets/2019_2_validation_assignments_tab_page.png)

### Change path to UNIX format
UNIX format is now supported for the change path task.
...
![Universal theme](/docs/blog/assets/2019_2_universal_theme.png)

### Universal Theme - Conditional layout
The screen for Conditional layouts is extended to support Universal colors. For Universal applications, it is no longer possible to set a complete font. 
...
![Universal theme - Conditional layout](/docs/blog/assets/2019_2_universal_theme_conditional_layout.png)
...
* **Longer processflow name possible** - Processflow names can now be 100 characters long instead of 50. What's important to keep in mind is that the combined length of the process flow and process action may not exceed the maximum length of 120 characters. Added a validation to check this.
...
* **Data conversion - identity seed** - The identity seed in an upgrade table will now always remain the same or willb e set to a higher value after an upgrade on the SQL Server database platform. This prevents identity values of deleted rows from being reused. This could occur when a record with the highest identity value was deleted before the upgrade.
* **Replaced icons in SF** - The Software Factory has got a new look by replacing all used icons for new SVG icons. These icons are in the Material style to better suit the Universal GUI in the future.  
...
* **New project versions now contain the last code history** - Last code history will now be copied to the new project version with the copy project version task
...
* **Grouped tabpages for Applications** - The Preferences and Effective rights tabpages in the Applications screen are now grouped tab pages. Both show a Group and User tabpage.
...
* **Added effective rights for user groups** - Added effective rights for user groups. This can be found in the pages 'User groups' and 'Applications'.
...
* **Custom prefilters and status issues resolved** - Issues regarding table prefilter status in IAM are resolved. Also the conversion errors while modifying custom user prefilters.
