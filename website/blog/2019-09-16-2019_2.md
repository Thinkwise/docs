...
* Requirements can be placed anywhere in these views and is categorized into various types
...
Unit test now supports processes and tasks. It is also possible to select a data set as mock data for the unit test.
...
The screen for Conditional layouts is extended to support Universal colors. 
 
For Universal applications, it is no longer possible to set a complete font, but only to modify the color, bold or italic of the font. 
This makes it easier to create a uniform application with the same font face and font size.  
 A user can choose between dark or light theme, therefore both colors should be filled and tested so they are well readable with the selected color and background.
...
* Process flows
  * Combined length of process flow and process action is too long
...
* **SQL Server upgrade identity seed** - The identity seed in the new table will now be set to the previous tables' identity seed if the previous identity seed was higher. Identity values due to deleted records will not be reused.
...
* **Grouped tabpages for Applications** - The Preferences and Effective rights tabpages in the Applications screen are now grouped tab pages. Both now show a Group and User tabpage.
...
| SF - Old table            | SF - Old column          | SF - New table            | SF - New column                    |
| ------------------------- | ------------------------ | ------------------------- | ---------------------------------- |
...
| IAM - Old table            | IAM - Old column      | IAM - New table           | IAM - New column                    |
| -------------------------- | --------------------- | ------------------------- | ----------------------------------- |
| conditional_layout         | -                     | conditional_layout        | background_type                     |
| conditional_layout         | -                     | conditional_layout        | background_color_light              |
| conditional_layout         | -                     | conditional_layout        | background_color_dark               |
